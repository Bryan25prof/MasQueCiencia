...
### HOTFIX-11 — Intro cinematográfica de ingreso + ajustes de identidad de marca
Integra al proyecto real la "Variante D", ganadora de una ronda de 4 prototipos aislados evaluados por el docente antes de tocar el proyecto principal (proceso documentado en las conversaciones de diseño de este sprint — cada variante se construyó y probó en un archivo HTML autocontenido, sin integrar nada hasta recibir aprobación expresa).

- **Intro cinematográfica de ingreso** (`css/intro.css` + `js/shared/intro.js`, activados desde `index.html`): consola de arranque ("INICIANDO SISTEMA MQC...", verificación de módulos) → lluvia de símbolos químicos cayendo por toda la pantalla → un escáner cruza de izquierda a derecha "leyendo y bloqueando" cada columna a su paso → onda de choque + "IDENTIDAD VERIFICADA" → laboratorio Cosmos MQC se revela con Photon (representación provisional) → "Todo listo." / "Comencemos." → identidad de marca (crédito "Lic. Bryan Chavarría C." + MÁSQUECIENCIA + "Tú defines tu éxito, comienza ahora.") → botón "Entrar al laboratorio".
- **Se muestra una única vez por navegador** (localStorage `mqc_intro_seen_v1`), con botón "Saltar intro" y control de sonido siempre visibles. Si localStorage no está disponible, la intro simplemente se repite cada vez — nunca bloquea el acceso a la plataforma.
- **Sonido sintetizado en el momento** (Web Audio API, sin archivos externos): beeps de consola, barrido del escáner, golpe grave del congelamiento, arpegio suave de materialización — todo dentro de `try/catch`, así que si el navegador bloquea audio, la experiencia continúa sin sonido.
- **Decisión de arquitectura importante — por qué Photon aparece en forma provisional y no con el componente oficial:** se evaluó usar el `window.Photon` real (`js/shared/photon.js`), pero es una instancia única por diseño (`Photon.mount()` destruye cualquier instancia previa). Si un estudiante con perfil ya activo ve esta intro, `js/app.js` puede montar el Photon real del sidebar en paralelo mientras la intro todavía está en pantalla, y ambos montajes competirían por la misma instancia — dejando al Photon del sidebar sin aparecer tras cerrar la intro. La representación provisional (núcleo + 3 órbitas, con los mismos colores de marca) no depende de esa instancia y por lo tanto no puede chocar con nada. El componente oficial de Photon queda completamente intacto.
- **Identidad de marca — rótulo fijo**: por pedido explícito del docente, la línea de ruta académica del sidebar/topbar (antes dinámica: "QUÍMICA 10.º" en secciones de Décimo, "QUÍMICA 11.º" en 'grade11') pasa a ser un rótulo fijo, **"QUÍMICA INTERACTIVA 10.º Y 11.º"**, sin importar la sección activa — la plataforma ya cubre ambos grados y la marca ahora lo refleja siempre. Cambio en `js/core/router.js` (`_updateBrandRoute`) y en los 2 valores estáticos equivalentes de `index.html` (evita cualquier parpadeo antes de que cargue el JS).
- **"Acerca de la Plataforma" — ícono e texto actualizados**: el ícono genérico ℹ️ se reemplazó por ⚛️ (átomo, coherente con el propio logo de MQC) con un glow cian/violeta pulsante exclusivo de esta sección (`css/main.css` → `.placeholder-icon-glow`, no afecta a ninguna otra página placeholder). El texto descriptivo se actualizó con el nuevo copy del docente sobre Química Interactiva 10.º y 11.º como "más que una plataforma, una experiencia nueva para aprender ciencia".
- **Verificación real ejecutada:** `node --check` limpio en el 100% de los `.js` del proyecto; verificación de balance de `<div>` y ausencia de IDs duplicados en `index.html`; confirmación de que las 17 referencias de elementos usadas por `js/shared/intro.js` existen todas en el HTML; comparación binaria contra el ZIP inmediatamente anterior (post-HOTFIX-10) confirmando que **únicamente 5 rutas cambiaron en todo el proyecto** — 2 archivos nuevos (`css/intro.css`, `js/shared/intro.js`) y 3 modificados (`index.html`, `css/main.css`, `js/core/router.js`) — el resto del proyecto, incluyendo el componente oficial de Photon, el sistema de perfiles, XP, medallas, y todas las unidades de Química 10.º y 11.º, quedan exactamente iguales.
- Archivos nuevos: `css/intro.css`, `js/shared/intro.js`. Archivos modificados: `index.html`, `css/main.css`, `js/core/router.js`.

### HOTFIX-12 — Sidebar compacto, consistencia de íconos y afinado de la intro
Tres ajustes puntuales sobre el trabajo de HOTFIX-11, aplicados directamente al proyecto real tras aprobación de la maqueta comparativa.

- **Sidebar compacto**: la tarjeta de usuario pasa de 4 líneas apiladas a una sola fila (avatar + nombre + XP a la izquierda, nivel + XP numérico a la derecha), y los 3 accesos rápidos (Buscar, Glosario, Accesibilidad) pasan de filas con nombre+descripción a 3 chips compactos de ícono + una palabra, con la descripción completa disponible como tooltip (`title=""`). Juntos, estos 2 bloques bajan de ~250px a ~130px de alto, devolviéndole ese espacio a la navegación scrollable (9 accesos), que es la más usada. Mismos IDs de siempre en ambos casos (`sidebar-user-name`, `sidebar-xp-fill`, `qi-launch-search`, etc.) — ni `js/app.js` ni `js/shared/launcher.js` necesitaron ningún cambio.
- **Ícono de "Acerca de" — consistencia**: el ícono del ítem de navegación en el sidebar (`ℹ️`, nunca actualizado en HOTFIX-11) ahora es el mismo átomo (`⚛️`) que ya se usa en el banner de esa página — un solo ícono consistente en toda la plataforma para esa sección, en vez de dos íconos distintos.
- **Intro cinematográfica — 2 afinados**:
  - El texto que aparece al congelarse la lluvia cambia de "IDENTIDAD VERIFICADA" a **"CARGANDO EXPERIENCIA"**.
  - La lluvia de elementos deja de ser 100% azar: **3 columnas están sembradas de antemano con las letras M · Q · C**, repartidas proporcionalmente (20%/50%/80% del ancho, recalculado en cada carga según el tamaño de pantalla — funciona igual en un teléfono angosto que en un monitor grande). Mientras caen, esas fichas se ven exactamente igual que cualquier otro elemento químico (mismo símbolo aleatorio) — la palabra **"MQC" se revela letra por letra, en orden, exactamente cuando el escáner (que ya se movía de izquierda a derecha) las va congelando a su paso**, con un realce violeta distinto del blanco genérico del resto de fichas congeladas.
- **Verificación real ejecutada:** `node --check` limpio en el 100% de los `.js`; sin IDs duplicados ni `<div>` desbalanceados en `index.html`; simulación en Node de la siembra de columnas M/Q/C en 7 anchos de pantalla distintos (320px a 1920px) confirmando que las 3 letras siempre caen dentro de rango y nunca chocan entre sí; comparación binaria contra el ZIP inmediatamente anterior (post-HOTFIX-11) confirmando que **únicamente 4 archivos cambiaron en todo el proyecto** — `index.html`, `css/main.css`, `css/intro.css`, `js/shared/intro.js` — ningún archivo nuevo, nada más se tocó.

### SPRINT DE AFINAMIENTO PRE-PNE — Auditoría + mejoras didácticas
Sprint de pulido previo al Centro Nacional de Preparación PNE. Solo mejora calidad, variación, consistencia y corrige un bug real confirmado por auditoría — no toca arquitectura, Photon, perfiles, XP, medallas, multigrado ni contenido pedagógico de ninguna unidad.

- **Auditoría del Atlas Químico (Parte I)**: informe completo en `INFORME_AUDITORIA_ATLAS_QUIMICO.md` con tabla de desbloqueo de las 15 fichas. Bug real encontrado y corregido: Alqueno y Alquino no tenían ningún camino de desbloqueo en todo el proyecto (la Unidad III los enseña a fondo pero nunca llamaba al Atlas) — cableado en `js/units/grade11/g11-u03.js` sin tocar contenido. Simulación 100% real: antes 2/15 fichas bloqueadas permanentemente, después **0/15**.
- **Maestro del Mol Premium (Parte II)**: de 2 tipos de conversión a **8** (masa⇄mol⇄partículas en ambas direcciones + 2 conversiones encadenadas), pool de 11 valores variados con anti-repetición consecutiva garantizada (antes se podían repetir 2-3 rondas seguidas con la misma cantidad).
- **Distractores conceptuales (Parte III)**: error de exponente (×10²²/×10²⁴), coeficiente de Avogadro redondeado distinto (6.02 vs. 6.022), errores de magnitud (×10, ÷10) — en vez de números al azar.
- **2 bugs de aleatoriedad autodetectados y corregidos** durante las pruebas de este mismo sprint (no reportados por nadie): duplicados en los tipos de conversión encadenada, y una respuesta correcta que siempre caía en la primera posición en 2 de los 8 tipos.
- **Estequiometría (Parte IV)**: ecuaciones del Balanceador con presentación profesional — flecha más grande, coeficientes destacados, mayor separación visual. Cero cambios en la lógica de balanceo.
- **Escáner Molecular (Parte V)**: resalte visual propio (glow) para el grupo funcional exacto encontrado, distinto del resto de grupos de la molécula. Nuevo dato "también aparece en…" conectando con otras moléculas reales del mismo banco.
- **Preguntas tipo PNE (Parte VI)**: 8 preguntas nuevas de razonamiento real (¿cuál NO aparece?, ¿cuántos aparecen?, seleccione todos los presentes, ¿cuál contiene simultáneamente…?), banco 48→56, cobertura PNE 56/56.
- **Validación general (Parte VII)**: `node --check` en el 100% de los `.js`, módulos protegidos confirmados idénticos por `diff` binario (14 archivos verificados: Photon, perfiles, Gamification, selector de grado, las 9 unidades de Química 10.º y 2 de Química 11.º), CSS balanceado en los 4 archivos (723 llaves de apertura = 723 de cierre), 40.000 ejercicios de conversión simulados sin fallos con distribución uniforme de posición, 1.000 exámenes balanceados simulados sin fallos.
- Archivos modificados: `js/units/grade11/g11-u03.js`, `js/units/unit-06.js`, `js/shared/molecule-renderer.js`, `js/units/grade11/g11-u04.js`, `js/data/grade11/preguntas-g11-u04.js`, `js/data/grade11/banco-pne-g11-u04.js`.

### HOTFIX-13 — 3 problemas reales reportados en dispositivo (iPhone) y auditados
Reportados con evidencia fotográfica real por el docente en un iPhone físico. Los 3 se auditaron primero (causa raíz confirmada leyendo el código, no adivinada) y luego se corrigieron.

- **La intro se repetía menos de lo esperado**: antes se marcaba "vista" apenas se hacía clic en "Entrar al laboratorio", incluso si el estudiante nunca llegaba a crear un perfil real (cerraba la pestaña, recargaba antes de terminar). Ahora la intro se repite en cada carga mientras el navegador no tenga ningún perfil real creado (`MQCProfiles.count() === 0`), y deja de aparecer para siempre en el instante en que se completa el primer perfil de verdad.
- **Formulario de crear perfil cortado en iPhone (perfecto en Android)**: causa raíz confirmada — `.mqc-gate-atmosphere` (el contenedor de toda la pantalla) tenía `overflow:hidden`, mientras que `.mqc-gate-stage` (el contenido de adentro) tenía su propio `overflow-y:auto` anidado. Esa combinación es un bug conocido de iOS Safari: el scroll interno deja de responder al tacto, dejando inalcanzable todo lo que no entra en el primer vistazo (el grid de insignias, el botón de crear perfil). Ya se habían intentado 2 arreglos anteriores (HOTFIX-07, HOTFIX-09) ajustando propiedades táctiles del contenedor interno sin resolverlo del todo — la causa real era la anidación en sí, no una propiedad faltante. Corregido: ahora hay un único contenedor con scroll (el exterior), sin anidar. Mismo arreglo aplicado también a `js/shared/profiles-ui.js` (`_panel()`), usado por otros paneles del perfil (línea de tiempo, reflexiones), que tenían el mismo patrón.
- **El audio no sonaba al inicio de la intro**: esto no era un bug de código — es una política de seguridad de TODOS los navegadores modernos (Chrome, Safari, Firefox), que bloquean cualquier sonido que no venga de un gesto real y directo del usuario; ningún código puede saltarse esto. Se agregó una compuerta breve "Toca para comenzar, con sonido activado" al inicio de la intro: el mismo toque que la cierra es el gesto real que desbloquea el audio, así que la secuencia completa (incluido su primer sonido) arranca recién después, con el sonido ya activo desde el primer frame.
- **Verificación real ejecutada:** `node --check` en el 100% de los `.js`; balance de llaves confirmado en los 4 archivos CSS; comparación binaria contra el ZIP anterior confirmando que únicamente 5 archivos cambiaron.
- Archivos modificados: `index.html`, `css/main.css`, `css/intro.css`, `js/shared/intro.js`, `js/shared/profiles-ui.js`.

...
(function () {
  ...
  function startLevel(idx){const lv=GAME_LEVELS[idx];if(!lv)return;game={idx,lv,casos:shuffle(lv.casos),i:0,score:0,correct:0,answered:false};/*FIX-XP-02: sin XP por solo iniciar*/drawCase();}
  function drawCase(){
  ...
  function finishLevel(){
    ...
    const st=gameState();const done=st.done.slice();const yaAprobadoAntes=done.includes(game.lv.id);if(passed&&!yaAprobadoAntes)done.push(game.lv.id);
    ...
    patchUnit({gameScore:Math.max(st.best,score),gameLevels:done,gameLevelsPlayed:jugados});
    ...
    if(score>st.best)awardXP('game-highscore');

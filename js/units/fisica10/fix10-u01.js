/* ================================================================
   ...
   ================================================================
   ⚠️ CONTENIDO PROVISIONAL: la arquitectura (pestañas, XP, progreso,
   simuladores, juego, examen, misión) ya está completa y probada,
   pero el TEXTO de cada tema, las situaciones de los simuladores y
   los escenarios del juego son marcadores de posición ("PROVISIONAL")
   hasta procesar el libro fuente "Física 10° — Un enfoque práctico".
   NO se improvisó contenido académico real fuera de una fuente —
   por eso se dejó explícitamente marcado como pendiente en vez de
   inventar afirmaciones de Física como si fueran definitivas.
   ...
   nunca a las funciones de Química.
...
(function () {
  ...
  const TEMAS = [
    ...
    { id: 't2', titulo: 'Física teórica y experimental',
    ...
    { id: 't3', titulo: 'Del universo clásico a la relatividad',
    ...
    { id: 't5', titulo: '¿Dónde está la Física?',
    ...
    { id: 't6', titulo: 'Física, tecnología y sociedad',
  ...
  const ELEMENTOS_SIM2 = [
    ...
    { id: 'satelite', nombre: 'Satélite de comunicaciones', areas: 'Mecánica orbital + Electromagnetismo' }
  ];
  function renderSim2() {
    return `
      <div>
        <p style="color:var(--text-secondary);margin-bottom:1rem">Tocá cada elemento para ver qué área(s) de la Física intervienen en su funcionamiento.</p>
  ...
  function renderSim3() {
    return `
      <div>
        <p style="color:var(--text-secondary);margin-bottom:1rem">Un mismo movimiento se describe distinto según quién lo observe. Elegí un observador:</p>
        <div style="display:flex;gap:.6rem;margin-bottom:1rem">
          <button class="btn btn-ghost" data-sim3-obs="tierra">Observador en Tierra</button>
          <button class="btn btn-ghost" data-sim3-obs="vehiculo">Observador dentro del vehículo</button>
        </div>
        <p id="sim3-feedback" style="font-size:.9rem;min-height:3em"></p>
      </div>`;
  }
  ...
  function renderTeoria(unit, uData) {
    ...
    return `
      <div class="teoria-panel">
        ${TEMAS.map((t, i) => `
          <div class="acc-item">
            <button class="acc-header" data-acc="${i}">
              <span>${leidos.includes(t.id) ? '✅' : '⬜'} ${t.titulo}</span>
              <span class="acc-chevron">▾</span>
            </button>
            <div class="acc-body" data-acc-body="${i}" style="display:none">
              <p style="color:var(--text-secondary);line-height:1.7">${t.texto}</p>
              <button class="btn btn-ghost btn-sm" data-read="${i}" data-tema="${t.id}">
                ${leidos.includes(t.id) ? 'Ya leído ✓' : 'Marcar como leído'}
              </button>
            </div>
          </div>
        `).join('')}
  ...
  function bindTeoria(unit, uData) {
    ...
    document.querySelectorAll('[data-read]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        const container = document.querySelector('.teoria-panel').parentElement;
  ...
  function renderSimuladores(unit, uData) {
    ...
    const metas = [
      ...
      { id: 'sim2', titulo: '📡 ¿Dónde está la Física?', desc: 'Tocá cada tecnología (GPS, puente, láser...) y descubrí qué área de la Física interviene.' },
      { id: 'sim3', titulo: '🔄 Cambia el observador', desc: 'Un mismo movimiento visto desde dos marcos de referencia distintos.' }
    ];
    return `
  ...
  function bindSimuladores(unit, uData) {
    document.querySelectorAll('[data-sim-abrir]').forEach(btn => {
      btn.addEventListener('click', () => { _simActivo = btn.getAttribute('data-sim-abrir'); _sim1Idx = 0; _rerenderSimTab(unit); });
    });
    ...
    /* Sim3 — Cambia el observador */
    document.querySelectorAll('[data-sim3-obs]').forEach(btn => {
      btn.addEventListener('click', () => {
        const obs = btn.getAttribute('data-sim3-obs');
        const fb = document.getElementById('sim3-feedback');
        if (fb) {
          fb.textContent = obs === 'tierra'
            ? 'Desde la Tierra, el vehículo se ve avanzar a su velocidad real, y cualquier objeto lanzado dentro de él parece sumar o restar esa velocidad.'
            : 'Para quien va DENTRO del vehículo, este está quieto — un objeto lanzado hacia arriba sube y baja igual que si estuviera parado, sin importar qué tan rápido se mueva el vehículo por fuera.';
        }
        markSimDone('sim3');
      });
    });
  ...
  /* ================================================================
     JUEGO — "Detective de la Física" (3 niveles STUB). Mismo patrón
     corregido que unit-01.js: SIN XP al solo iniciar un nivel, y
     game-won/game-played UNA sola vez por nivel (no por intento).
     ================================================================ */
  const NIVELES_JUEGO = [
  ...
  function renderExamen(unit, uData) {
    ...
    return `
      <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius-lg);padding:1.5rem;text-align:center;max-width:520px">
        ...
        <p style="color:var(--text-muted);font-size:.78rem">⚠️ Banco provisional de ${banco.length} preguntas — se ampliará a 50+ con el libro fuente.</p>

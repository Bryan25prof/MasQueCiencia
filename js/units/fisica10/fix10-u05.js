...
(function () {
  ...
  function renderJuego(unit, uData) {
    ...
    if (_juegoNivelActivo) {
      ...
      return `
        <div class="juego-panel">
          ...
          ${_svgGrafica(n.puntos, { tMax: Math.max.apply(null, n.puntos.map(p => p.t)), yLabel: n.yLabel, color: n.color || '#7B2FFF', marcasT: n.puntos.map(p => p.t) })}
          <p style="color:var(--text-muted);font-size:.82rem;margin:.6rem 0">💡 ${n.pista}</p>
    ...
    return `
      <div class="juego-panel">
        <h3>🕵️ Misión: Reconstruye el Viaje</h3>
        <p style="color:var(--text-secondary);font-size:.85rem">MQC recibió gráficas de movimiento sin su descripción — reconstruí qué pasó en cada una.</p>
        <div style="display:grid;gap:.8rem;margin-top:1rem">
          ${NIVELES_JUEGO.map((n, i) => `
            <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius-md);padding:1rem">
              <p style="margin:0 0 .4rem"><strong>Nivel ${i + 1}</strong></p>
